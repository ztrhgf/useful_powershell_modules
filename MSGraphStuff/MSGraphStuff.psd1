...
RequiredModules = @('Az.Accounts', 
               'MSAL.PS', 
               'DependencySearch', 
               @{ModuleName = 'CommonStuff'; ModuleVersion = '1.0.23'; })
...
PrivateData = @{
    ...
    PSData = @{
        ...
        ReleaseNotes = '
            1.0.15
                CHANGED
                    Invoke-GraphBatchRequest - added parallel processing (in PSH Core only) for significantly better performance
            1.0.14
                FIXED
                    Invoke-GraphBatchRequest - fixed batch request flattening when invoked in pipeline
            1.1.13
                CHANGED
                    Invoke-GraphBatchRequest - new parameters dontFollowNextLink and separateErrors
                    New-GraphBatchRequest - check that Id does not contain "\" character
                                          - uri alias for url parameter
                    Get-CodeGraphPermissionRequirement - added support for detecting New-GraphBatchRequest api calls
                FIXED
                    Invoke-GraphBatchRequest - fixed the output when error is not a JSON object
            1.1.12
                FIXED
                    Invoke-GraphBatchRequest - 204 http response was not handled correctly
                CHANGED
                    Invoke-GraphAPIRequest - better http response detection
            1.1.11
                CHANGED
